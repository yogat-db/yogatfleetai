              ...
              >
                ...
                <div style={styles.meta}>
                  <div style={styles.metaItem}>
                    <PoundSterling size={14} /> <span>Est. {job.budget ? `£${job.budget}` : 'TBD'}</span>
                  </div>
                  <div style={styles.metaItem}>
                    <MapPin size={14} /> <span>{job.location || 'Remote/TBD'}</span>
                  </div>
                </div>
                {job.vehicle && (
...
function LoadingSkeleton() {
  return (
    <div style={styles.page}>
      <div style={{ width: '300px', height: '40px', background: '#1e293b', borderRadius: '8px', marginBottom: '40px' }} />
      <div style={styles.grid}>
        {[1, 2, 3].map(i => <div key={i} style={{ ...styles.card, height: '280px', opacity: 0.5, background: '#0f172a' }} />)}
      </div>
    </div>
  );
}
...
const styles: Record<string, React.CSSProperties> = {
  ...
  vehicleTag: { display: 'inline-flex', alignItems: 'center', gap: '6px', background: '#1e293b', padding: '4px 10px', borderRadius: '6px', fontSize: '12px', color: '#94a3b8', marginBottom: '20px', width: 'fit-content' },
  ...
  applyButton: { background: theme.colors.primary, border: 'none', padding: '8px 16px', borderRadius: '8px', fontWeight: 700, cursor: 'pointer', color: '#020617', fontSize: '13px' },
  ...
  registerButton: { width: '100%', padding: '12px', background: theme.colors.primary, border: 'none', borderRadius: '12px', fontWeight: 700, cursor: 'pointer' },
